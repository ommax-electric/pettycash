...
export default function QuotationToolsView({
  ...
  // Soft warning tracking for unsaved changes
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (hasUnsavedChanges) {
        e.preventDefault();
        e.returnValue = 'You have unsaved changes in Quotation Tools. Are you sure you want to leave?';
  ...
  const [pendingTabChange, setPendingTabChange] = useState<MasterConfigTab | null>(null);
  ...
  const executeDiscard = () => {
    try {
      const parsed = JSON.parse(lastSavedJson);
      setConfig(parsed);
    } catch (err) {
      console.error('Failed to revert master config', err);
    }
  ...
  const executeResetToDefaults = () => {
    const stringified = JSON.stringify(DEFAULT_QUOTATION_MASTER_CONFIG);
  ...
  const handleTabClick = (targetTab: MasterConfigTab) => {
  ...
  return (
    <div className="space-y-6">
      {/* Unsaved Tab Change Soft Warning Modal */}
      {pendingTabChange && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-150 space-y-4">
            <div className="flex items-start gap-3.5">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-black text-slate-900 tracking-tight">
                  Unsaved Changes in Configuration
                </h3>
                <p className="text-xs text-slate-600 mt-1 leading-relaxed">
                  You have made changes in the <strong className="text-slate-800 font-bold">{tabs.find(t => t.id === activeTab)?.label}</strong> tab that are not saved yet. Would you like to save them before moving to <strong className="text-slate-800 font-bold">{tabs.find(t => t.id === pendingTabChange)?.label}</strong>?
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 pt-3 border-t border-slate-100">
              <button
                ...
                className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-xs transition-colors cursor-pointer text-center"
              >
                Stay on Tab
              </button>
              <button
                ...
                className="px-3.5 py-2 bg-slate-100 hover:bg-rose-50 hover:text-rose-700 text-slate-700 font-bold rounded-xl text-xs transition-colors cursor-pointer text-center border border-slate-300 hover:border-rose-300"
              >
                Discard & Switch
              </button>
              <button
                ...
                className="px-4 py-2 bg-[#f7b944] hover:bg-amber-400 text-slate-950 font-black rounded-xl text-xs transition-all flex items-center justify-center gap-1.5 shadow-xs cursor-pointer text-center"
              >
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-150 space-y-4">
            <div className="flex items-start gap-3.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-600 mt-1 leading-relaxed">
                  Are you sure you want to discard your unsaved configuration changes? Any edits made since the last save will be reverted.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
              ...
              <button
                ...
                className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl text-xs transition-all flex items-center gap-1.5 shadow-xs cursor-pointer"
              >
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-150 space-y-4">
            <div className="flex items-start gap-3.5">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-black text-slate-900 tracking-tight">
                  Reset Master Configuration to Factory Defaults?
                </h3>
                <p className="text-xs text-slate-600 mt-1 leading-relaxed">
                  This will restore all default system dropdowns, standard pricing rates, clauses, and template text to their original factory defaults.
                </p>
              </div>
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {hasUnsavedChanges && (
                <span className="inline-flex items-center gap-1 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full bg-amber-100 text-amber-900 border border-amber-300 animate-pulse">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {hasUnsavedChanges && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-700 rounded-xl text-xs font-bold transition-all cursor-pointer"
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-4.5 py-2 rounded-xl text-xs font-black transition-all cursor-pointer shadow-xs ${
              saveSuccess
                ...
                : hasUnsavedChanges
                  ? 'bg-[#f7b944] hover:bg-amber-400 text-slate-950 ring-2 ring-amber-400 ring-offset-1 shadow-md font-black animate-pulse'
